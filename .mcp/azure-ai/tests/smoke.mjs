...
// ejecuta el smoke() co-ubicado de cada tool. No hace llamadas reales a Azure AI con
// costo (azure_ai_status sin checkEndpoint, y los demás validan errores tempranos).
//
// Ejecución: node .mcp/azure-ai/tests/smoke.mjs
...
try {
  ...
  check(
    ...
    init.result?.serverInfo?.name === 'azureAi',
  ...
  check('tools/list incluye los 7 tools de azure-ai', expected.length === 7, `total=${expected.length}`);
