...
// ejecuta el smoke() co-ubicado de cada tool. No hace llamadas reales a Gemini/Veo con
// costo (google_ai_status sin checkEndpoint, y los demás validan errores tempranos).
//
// Ejecución: node .mcp/google-ai/tests/smoke.mjs
...
try {
  ...
  check(
    ...
    init.result?.serverInfo?.name === 'googleAi',
  ...
  check('tools/list incluye los 3 tools de google-ai', expected.length === 3, `total=${expected.length}`);
