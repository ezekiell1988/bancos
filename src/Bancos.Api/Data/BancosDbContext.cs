...
{
    public DbSet<Owner> Owners => Set<Owner>(); public DbSet<Currency> Currencies => Set<Currency>(); public DbSet<Account> Accounts => Set<Account>(); public DbSet<AccountAuxiliary> AccountAuxiliaries => Set<AccountAuxiliary>(); public DbSet<Import> Imports => Set<Import>(); public DbSet<ImportProgress> ImportProgress => Set<ImportProgress>(); public DbSet<ImportFingerprint> ImportFingerprints => Set<ImportFingerprint>(); public DbSet<ImportTemplatePattern> ImportTemplatePatterns => Set<ImportTemplatePattern>(); public DbSet<Category> Categories => Set<Category>(); public DbSet<ClassificationRule> ClassificationRules => Set<ClassificationRule>(); public DbSet<ClassificationTag> ClassificationTags => Set<ClassificationTag>(); public DbSet<Transaction> Transactions => Set<Transaction>(); public DbSet<CardStatement> CardStatements => Set<CardStatement>(); public DbSet<CreditFinancing> CreditFinancings => Set<CreditFinancing>(); public DbSet<LoanStatement> LoanStatements => Set<LoanStatement>(); public DbSet<LoanPayment> LoanPayments => Set<LoanPayment>(); public DbSet<JournalEntry> JournalEntries => Set<JournalEntry>(); public DbSet<JournalLine> JournalLines => Set<JournalLine>(); public DbSet<Reconciliation> Reconciliations => Set<Reconciliation>(); public DbSet<ReconciliationTransaction> ReconciliationTransactions => Set<ReconciliationTransaction>(); public DbSet<ExchangeRate> ExchangeRates => Set<ExchangeRate>(); public DbSet<ForeignExchangeClosing> ForeignExchangeClosings => Set<ForeignExchangeClosing>(); public DbSet<ForeignExchangeClosingLine> ForeignExchangeClosingLines => Set<ForeignExchangeClosingLine>(); public DbSet<ReportPeriod> ReportPeriods => Set<ReportPeriod>(); public DbSet<AuditLog> AuditLogs => Set<AuditLog>();
    ...
    protected override void OnModelCreating(ModelBuilder b)
    {
        ...
        b.Entity<ImportFingerprint>().HasIndex(x => x.Hash).IsUnique(); b.Entity<ImportTemplatePattern>().HasIndex(x => x.SignatureHash).IsUnique(); b.Entity<Import>().HasIndex(x => x.ContentHash);
        b.Entity<ImportProgress>().HasKey(x => x.ImportId);
        b.Entity<ImportProgress>().Property(x => x.Stage).HasMaxLength(40);
        b.Entity<ImportProgress>().HasOne(x => x.Import).WithOne().HasForeignKey<ImportProgress>(x => x.ImportId).OnDelete(DeleteBehavior.Cascade);
        b.Entity<ImportProgress>().ToTable(t =>
        {
        ...
        b.Entity<CardStatement>().HasIndex(x => new { x.AccountAuxiliaryId, x.CardNumberMasked, x.StatementDate }).IsUnique();
        b.Entity<CardStatement>().HasOne(x => x.Import).WithMany().HasForeignKey(x => x.ImportId).OnDelete(DeleteBehavior.Restrict);
