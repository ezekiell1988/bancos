...
export class ImportsPage {
  ...
  private preview(file: File) { this.items.update(items => [...items, { file, template: 'unknown', label: 'Analizando…', status: 'unsupported', message: null }]); this.api.preview(file).subscribe({ next: preview => this.items.update(items => items.map(item => item.file === file ? { ...preview, file } : item)), error: () => this.items.update(items => items.map(item => item.file === file ? { ...item, label: 'No se pudo analizar', status: 'needs-type', message: 'Selecciona el tipo de archivo.' } : item)) }); }
