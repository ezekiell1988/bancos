...
export class ImportsPage {
  ...
  submit(event: SubmitEvent) { event.preventDefault(); const items = this.readyItems(); if (!this.canSubmit()) return; this.saving.set(true); let left = items.length; this.message.set(''); items.forEach(item => this.api.upload(item.file, item.path, item.selectedTemplate).subscribe({ next: () => { if (!--left) { this.message.set('Archivos listos enviados para procesamiento.'); this.items.update(current => current.filter(entry => entry.status !== 'ready')); this.api.imports.reload(); this.saving.set(false); } }, error: () => { this.message.set('Un archivo no pudo cargarse.'); this.saving.set(false); } })); }
}
