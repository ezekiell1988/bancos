...
import { ImportItem, ImportPreview, ImportProgress, ImportsApi } from '../data-access/imports-api';
...
export class ImportsPage {
  ...
  statusLabel(status: ImportItem['status']) { return ({ Queued: 'En cola', Processing: 'Procesando', Completed: 'Completada', Failed: 'Fallida' } as const)[status]; }
  stageLabel(stage: string) { return ({ Queued: 'En cola', Starting: 'Iniciando', DetectingTemplate: 'Detectando formato', Extracting: 'Extrayendo', Classifying: 'Clasificando', Persisting: 'Guardando', Completed: 'Completada', Failed: 'Fallida' } as Record<string, string>)[stage] ?? 'Procesando'; }
  progressDescription(progress: ImportProgress) { return `${this.stageLabel(progress.stage)}: ${progress.current} de ${progress.total}, ${progress.percent}%`; }
  ...
  submit(event: SubmitEvent) { event.preventDefault(); const items = this.readyItems(); if (!this.canSubmit()) return; this.saving.set(true); this.message.set(''); this.messageKind.set(''); let left = items.length; let sent = 0; let duplicates = 0; let failed = 0; const successful = new Set<ReviewItem>(); const complete = () => { if (--left) return; this.items.update(current => current.filter(entry => !successful.has(entry))); this.api.imports.reload(); this.saving.set(false); if (failed) { this.messageKind.set('error'); this.message.set(`${sent} archivo(s) enviado(s); ${duplicates} ya existía(n); ${failed} no pudo/pudieron cargarse. Revisa los que quedaron en la lista e inténtalo nuevamente.`); } else if (duplicates) { this.messageKind.set('success'); this.message.set(`${sent} archivo(s) enviado(s); ${duplicates} ya estaba(n) importado(s). No se crearon duplicados.`); } else { this.messageKind.set('success'); this.message.set(`${sent} archivo(s) enviado(s) para procesamiento.`); } }; items.forEach(item => this.api.upload(item.file, item.path, item.entryIndex, item.selectedTemplate).subscribe({ next: () => { sent++; successful.add(item); complete(); }, error: (error: unknown) => { if (this.isDuplicateError(error)) duplicates++; else failed++; complete(); } })); }
  private isDuplicateError(error: unknown) { return error instanceof HttpErrorResponse && error.status === 400 && Array.isArray(error.error?.errors?.file) && error.error.errors.file.includes('An identical import already exists.'); }
}
